...
# ══════════════════════════════════════════════════════════════
    ...
    with tab2:
        if len(df_v) == 0:
            ...
            with c1:
                ...
                fig = px.bar(agg, x="type_vehicule", y="nb", color="annee",
    ...
    with tab4:
        ...
        with c1:
            ...
            df_c4["luminosite"] = df_c4["lum"].map(LUM_LABELS).fillna("Inconnu")
            lum_agg = df_c4.groupby(["annee","luminosite"]).size().reset_index(name="nb")
            fig = px.bar(lum_agg, x="luminosite", y="nb", color="annee",
                         barmode="group", title="Luminosité au moment de l'accident",
                         color_discrete_sequence=px.colors.qualitative.Set2)
        ...
        with c2:
            ...
            df_c4["meteo"] = df_c4["atm"].map(ATM_LABELS).fillna("Inconnu")
            atm_agg = df_c4.groupby(["annee","meteo"]).size().reset_index(name="nb")
            fig2 = px.bar(atm_agg, x="meteo", y="nb", color="annee",
                          barmode="group", title="Conditions météo",
                          color_discrete_sequence=px.colors.qualitative.Set2)
