"""
app.py – Dashboard Accidents Routiers France (2005–2024)  v4.0
=============================================================
Déployé sur Streamlit Cloud depuis GitHub.

  5 onglets disponibles sans aucun fichier externe :
    Vue d'ensemble · Tendances 2005-2024 · Véhicules impliqués
    Corrélations & Technologies · Données brutes

  Onglet Analyse BAAC 2018-2024 :
    Chargez vos fichiers CSV via la zone de dépôt dans l'onglet.
    Fichiers à télécharger sur data.gouv.fr
"""
...
with st.sidebar:
    ...
    data_dir = st.text_input(
        ...
        help="Laissez tel quel sur Streamlit Cloud. Modifiez si vous lancez l'app en local.",
    )
    ...
    if baac_ok:
        ...
        st.info(
            "ℹ️ Mode léger — fichiers BAAC non chargés\n\n"
            "Chargez vos fichiers CSV dans l'onglet **Analyse BAAC**."
        )
        ...
        with col_d:
            ...
            if baac_ok:
                try:
                    ...
                    st.info("Chargez les fichiers BAAC dans l'onglet Analyse BAAC pour voir le détail de gravité.")
            else:
                ...
                fig_pie.update_layout(height=380, showlegend=False)
                st.plotly_chart(fig_pie, use_container_width=True)
                st.caption("Estimation ONISR 2024 — chargez les fichiers BAAC pour le détail exact")
...
# ══════════════════════════════════════════════════════════════
# ONGLET 4 — ANALYSE BAAC 2018–2024 (upload depuis data.gouv.fr)
# ══════════════════════════════════════════════════════════════
elif onglet == "🔍 Analyse BAAC 2018–2024":
    st.title("🔍 Analyse Détaillée BAAC 2018–2024")

    # ── Helper : parser un fichier uploadé ──
    def parse_baac_file(uploaded_file):
        """Lit un fichier BAAC uploadé selon son nom."""
        import io
        ...
        # Extraire l'année depuis le nom de fichier (ex: usagers2024.csv → 2024)
        digits = ''.join(filter(str.isdigit, uploaded_file.name))
        df["annee"] = int(digits[-4:]) if len(digits) >= 4 else 0
        # Normaliser le nom de la colonne identifiant accident
    ...
    if source == "upload":
        ...
        Rendez-vous sur **[data.gouv.fr — Accidents corporels de la circulation](https://www.data.gouv.fr/datasets/bases-de-donnees-annuelles-des-accidents-corporels-de-la-circulation-routiere-annees-de-2005-a-2024)**
        et téléchargez les fichiers des années qui vous intéressent.
        """)

        with st.expander("Quels fichiers télécharger ?", expanded=True):
            st.markdown("""
            Pour chaque année (2018–2024), il y a **4 fichiers** à télécharger :

            | Table | Ce qu'elle contient | Nom du fichier (exemple 2024) |
            |-------|--------------------|-----------------------------|
            | **Caractéristiques** | Date, heure, commune, conditions météo | `caract2024.csv` |
            | **Lieux** | Type de route, profil, infrastructure | `lieux2024.csv` |
            | **Usagers** | Gravité, âge, sexe, équipement de chaque personne impliquée | `usagers2024.csv` |
            | **Véhicules** | Catégorie, manœuvre, type de choc de chaque véhicule | `vehicules2024.csv` |

            Vous pouvez charger **une seule année** ou **plusieurs à la fois**.
            Le dashboard s'adapte automatiquement.

            ⚠️ Les noms de fichiers varient légèrement selon les années :
            - 2024, 2023 : `caract2024.csv`, `caract2023.csv`
            - 2022, 2021 : `carcteristiques2022.csv`, `carcteristiques2021.csv`
            - 2020, 2019 : `caracteristiques2020.csv`, `caracteristiques2019.csv`
            - 2018 : `caracteristiques2018.csv` (encodage différent, géré automatiquement)
            """)

        # ── Zone de dépôt des fichiers ──────────────────────────
        st.markdown("### 2 · Glissez vos fichiers ici")
        uploaded_files = st.file_uploader(
            "Sélectionnez ou glissez-déposez vos fichiers CSV BAAC",
            type=["csv"],
            accept_multiple_files=True,
            help="Vous pouvez sélectionner plusieurs fichiers d'un coup avec Ctrl+clic (ou Cmd+clic sur Mac)"
        )

        if not uploaded_files:
            st.info("👆 Chargez au moins les 4 fichiers d'une année pour commencer l'analyse.")
            st.stop()

        # ── Parsing des fichiers uploadés ──────────────────────
        caracts_up, usagers_up, vehicules_up, lieux_up = [], [], [], []
        errors = []

        progress = st.progress(0, text="Chargement des fichiers…")
        for i, f in enumerate(uploaded_files):
            try:
                df_tmp = parse_baac_file(f)
                name = f.name.lower()
                if "caract" in name:
                    caracts_up.append(df_tmp)
                elif "usager" in name:
                    usagers_up.append(df_tmp)
                elif "vehic" in name:
                    vehicules_up.append(df_tmp)
                elif "lieu" in name:
                    lieux_up.append(df_tmp)
                else:
                    errors.append(f"⚠️ {f.name} — nom non reconnu (attendu : caract/usagers/vehicules/lieux)")
            except Exception as e:
                errors.append(f"❌ {f.name} — erreur : {e}")
            progress.progress((i + 1) / len(uploaded_files),
                              text=f"Chargement… {f.name}")
        progress.empty()

        # Résumé du chargement
        years_loaded = set()
        summary_cols = st.columns(4)
        for col, lst, label, emoji in zip(
            summary_cols,
            [caracts_up, usagers_up, vehicules_up, lieux_up],
            ["Caractéristiques", "Usagers", "Véhicules", "Lieux"],
            ["📋", "👤", "🚗", "📍"]
        ):
            if lst:
                yrs = sorted(set(df["annee"].iloc[0] for df in lst))
                years_loaded.update(yrs)
                col.success(f"{emoji} **{label}**\n\n{len(lst)} fichier(s)\nAnnées : {', '.join(map(str, yrs))}")
            else:
                col.warning(f"{emoji} **{label}**\n\nAucun fichier")

        if errors:
            with st.expander("⚠️ Fichiers non chargés"):
                for e in errors:
                    st.write(e)

        if not caracts_up:
            st.error("Les fichiers de **caractéristiques** sont obligatoires. Chargez au moins `caract2024.csv`.")
            st.stop()

        # Concaténer tout
        df_caract    = pd.concat(caracts_up,   ignore_index=True) if caracts_up    else pd.DataFrame()
        df_usagers   = pd.concat(usagers_up,   ignore_index=True) if usagers_up    else pd.DataFrame()
        df_vehicules = pd.concat(vehicules_up, ignore_index=True) if vehicules_up  else pd.DataFrame()
        df_lieux     = pd.concat(lieux_up,     ignore_index=True) if lieux_up      else pd.DataFrame()

    else:
        # Fichiers présents localement dans data_dir
...
# ══════════════════════════════════════════════════════════════
    ...
    tab_a, tab_b = st.tabs(["📥 Accès BAAC (upload)", "📊 Données intégrées"])

    with tab_a:
        if not baac_ok:
            st.info("Glissez vos fichiers BAAC dans l'onglet **Analyse BAAC** pour les analyser.")
        else:
