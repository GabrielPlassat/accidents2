...
# ══════════════════════════════════════════════════════════════
    ...
    sel = st.multiselect("Types de véhicules", all_cats,
    ...
    with col_a:
        ...
        df_comp = df_veh[df_veh["annee"].isin([2018, 2024]) & df_veh["categorie"].isin(sel)]
    ...
    with col_b:
        ...
        cats = [c for c in sel if c in pct.columns]
        figp = go.Figure()
        colors = px.colors.qualitative.Safe
        for i, cat in enumerate(cats):
            figp.add_trace(go.Scatter(
                x=pct.index, y=pct[cat], name=cat, mode="lines",
                stackgroup="one", fillcolor=colors[i % len(colors)],
                line=dict(color=colors[i % len(colors)], width=0.5),
            ))
        figp.update_layout(height=340, plot_bgcolor="white",
                           yaxis=dict(title="Part (%)"),
                           xaxis=dict(tickmode="linear", dtick=1),
                           legend=dict(orientation="h"))
        st.plotly_chart(figp, use_container_width=True)

    st.markdown("### Nouvelles mobilités : trottinettes & vélos assistés (depuis 2018)")
    df_nm = df_veh[df_veh["categorie"].isin(["EDP motorisé","EDP non motorisé","VAE/Vélo"])
                   & (df_veh["annee"] >= 2018)]
    if len(df_nm) > 0:
        fig4 = px.bar(df_nm, x="annee", y="nb", color="categorie", barmode="stack",
...
# ══════════════════════════════════════════════════════════════
    ...
    # ── Sélecteur d'années ────────────────────────────────────
    sel_years = st.multiselect("Années à analyser", annees_dispo,
                               default=annees_dispo, format_func=str)
    if not sel_years:
        st.warning("Sélectionnez au moins une année.")
        st.stop()

    df_c = df_c[df_c["annee"].isin(sel_years)]
    df_u = df_u[df_u["annee"].isin(sel_years)] if len(df_u) else df_u
    df_v = df_v[df_v["annee"].isin(sel_years)] if len(df_v) else df_v
    df_l = df_l[df_l["annee"].isin(sel_years)] if len(df_l) else df_l
    ...
    kc[3].metric("Années couvertes", len(sel_years))
    st.markdown("---")
    ...
    with tab1:
        if len(df_u) == 0:
            ...
            with c1:
                ...
                fig = px.line(monthly[monthly["annee"].isin(sel_years)],
                              x="mois", y="accidents", color="annee",
            ...
            with c2:
                try:
                    heure_series = pd.to_numeric(df_c["hrmn"].astype(str).str.split(":").str[0], errors="coerce")
                    hourly = df_c.assign(heure=heure_series).groupby(["annee","heure"]).size().reset_index(name="nb")
                    fig2 = px.line(hourly, x="heure", y="nb", color="annee",
                                   title="Accidents par heure de la journée",
                                   color_discrete_sequence=px.colors.qualitative.Set2)
            ...
            daily = df_c.assign(jour=pd.to_numeric(df_c["jour"], errors="coerce")).groupby(["annee","jour"]).size().reset_index(name="nb")
            daily["jour_label"] = daily["jour"].map(jours)
            # Normaliser par nb d'années pour comparer
            daily_pivot = daily.pivot_table(index="jour_label", columns="annee", values="nb")
            daily_pct = daily_pivot.div(daily_pivot.sum(axis=0), axis=1) * 100
            daily_pct = daily_pct.reindex(list(jours.values())).reset_index()
            daily_melt = daily_pct.melt(id_vars="jour_label", var_name="Année", value_name="Part (%)")
            fig3 = px.line(daily_melt, x="jour_label", y="Part (%)", color="Année",
                           title="Répartition des accidents par jour (%)",
                           markers=True,
    ...
    with tab2:
        if len(df_v) == 0:
            ...
            with c1:
                ...
                # Évolution par année : année en X, une ligne par type
                top10_list = list(top10)
                agg_line = (df_v2[df_v2["type_vehicule"].isin(top10_list)]
                            .groupby(["annee","type_vehicule"]).size().reset_index(name="nb"))
                fig = px.line(agg_line, x="annee", y="nb", color="type_vehicule",
                              title="Évolution des véhicules impliqués par type",
                              markers=True,
                              color_discrete_sequence=px.colors.qualitative.Set2,
                              labels={"nb": "Nb véhicules", "annee": "Année", "type_vehicule": ""})
        ...
        if len(dept_stats) == 0:
            ...
            with c2:
                fig2 = px.bar(dept_f.nlargest(15,"tues"), x="dep", y="tues",
                              title="Top 15 départements — Tués",
                              color="tues",
    ...
    with tab4:
        ...
        with c1:
            ...
            lum_agg = df_c.assign(luminosite=lum_series).groupby(["annee","luminosite"]).size().reset_index(name="nb")
            lum_pivot = lum_agg.pivot_table(index="annee", columns="luminosite", values="nb", aggfunc="sum").fillna(0)
            lum_pct = lum_pivot.div(lum_pivot.sum(axis=1), axis=0) * 100
            lum_melt = lum_pct.reset_index().melt(id_vars="annee", var_name="Luminosité", value_name="Part (%)")
            fig = px.line(lum_melt, x="annee", y="Part (%)", color="Luminosité",
                          title="Luminosité au moment de l'accident (% par année)",
                          markers=True,
                          color_discrete_sequence=px.colors.qualitative.Set2,
                          labels={"annee": "Année"})
            fig.update_layout(height=300, plot_bgcolor="white",
        ...
        with c2:
            ...
            atm_agg = df_c.assign(meteo=atm_series).groupby(["annee","meteo"]).size().reset_index(name="nb")
            meteo_pivot = atm_agg.pivot_table(index="annee", columns="meteo", values="nb", aggfunc="sum").fillna(0)
            meteo_pct = meteo_pivot.div(meteo_pivot.sum(axis=1), axis=0) * 100
            meteo_melt = meteo_pct.reset_index().melt(id_vars="annee", var_name="Météo", value_name="Part (%)")
            fig2 = px.line(meteo_melt, x="annee", y="Part (%)", color="Météo",
                           title="Conditions météo (% par année)",
                           markers=True,
                           color_discrete_sequence=px.colors.qualitative.Set2,
                           labels={"annee": "Année"})
            fig2.update_layout(height=300, plot_bgcolor="white",
        ...
        df_c4["type_collision"] = df_c4["col"].map(COL_LABELS).fillna("Inconnu")
        col_agg = df_c4.groupby("type_collision").size().reset_index(name="nb")
